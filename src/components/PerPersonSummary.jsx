...
import { PEOPLE, TOTAL_PEOPLE } from '../constants/people';

function PerPersonSummary({ expenses }) {
    ...
    <div className="bg-white rounded-lg shadow-sm p-6">
      ...
      <div className="mb-6 p-4 bg-blue-50 rounded-lg">
        ...
        <p className="text-xs text-gray-500 mt-1">
          {perPersonData.expenseCount} expense{perPersonData.expenseCount !== 1 ? 's' : ''} split among {TOTAL_PEOPLE} people
        </p>
      </div>
          ...
          >
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${
                ...
                index % 7 === 3 ? 'bg-yellow-500' :
                index % 7 === 4 ? 'bg-pink-500' :
                index % 7 === 5 ? 'bg-indigo-500' : 'bg-red-500'
              }`}></div>
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        <p className="text-xs text-gray-600 text-center">
          All expenses are split equally among all {TOTAL_PEOPLE} people
        </p>
      </div>
    </div>
  );
